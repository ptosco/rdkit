...
std::string generate_aligned_coords_deprecated(JSMol &self, const JSMol &templateMol, bool useCoordGen,
                                      bool allowOptionalAttachments,
                                      bool acceptFailure) {
  static bool deprecationMsgShown = false;
  if (!deprecationMsgShown) {
    deprecationMsgShown = true;
    std::cerr << "generate_aligned_coords(templateMol, useCoordGen, allowOptionalAttachments, acceptFailure) is deprecated, use generate_aligned_coords(templateMol, details) instead" << std::endl;
  }
  std::stringstream ss;
...
std::string generate_aligned_coords_deprecated(JSMol &self, const JSMol &templateMol, bool useCoordGen,
                                      bool allowOptionalAttachments) {
  return generate_aligned_coords_deprecated(self, templateMol, useCoordGen, allowOptionalAttachments, true);
}

std::string generate_aligned_coords_deprecated(JSMol &self, const JSMol &templateMol, bool useCoordGen) {
  return generate_aligned_coords_deprecated(self, templateMol, useCoordGen, false, true);
}

std::string generate_aligned_coords_helper(JSMol &self, const JSMol &templateMol, const emscripten::val &param) {
  if (param.typeOf().as<std::string>() == "boolean") {
    return generate_aligned_coords_deprecated(self, templateMol, param.as<bool>());
  } else if (param.typeOf().as<std::string>() != "string") {
    throw std::runtime_error("generate_aligned_coords expects a JSON string or a bool as parameter");
  }
...
EMSCRIPTEN_BINDINGS(RDKit_minimal) {
  ...
  class_<JSMol>("Mol")
      ...
      .function("generate_aligned_coords",
                select_overload<std::string(JSMol &, const JSMol &, const emscripten::val &)>(
      ...
      // DEPRECATED
      .function("generate_aligned_coords",
                select_overload<std::string(JSMol &, const JSMol &, bool, bool, bool)>(
                    generate_aligned_coords_deprecated))
