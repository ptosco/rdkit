...
extern "C" char *get_rxn(const char *input, size_t *pkl_sz,
                         const char *details_json) {
  std::unique_ptr<ChemicalReaction> rxn{MinimalLib::rxn_from_input(input, details_json)};
  if (!rxn) {
    *pkl_sz = 0;
    return NULL;
  }
...
std::unique_ptr<ExplicitBitVect> topological_torsion_fp_helper(const char *mol_pkl,
                                                   size_t mol_pkl_sz,
                                                   const char *details_json) {
  if (!mol_pkl || !mol_pkl_sz) {
    return nullptr;
  ...
  auto fp = AtomPairs::getHashedTopologicalTorsionFingerprintAsBitVect(mol, nBits);
  return std::unique_ptr<ExplicitBitVect>{fp};
}

std::unique_ptr<ExplicitBitVect> atom_pair_fp_helper(const char *mol_pkl,
                                                   size_t mol_pkl_sz,
                                                   const char *details_json) {
  if (!mol_pkl || !mol_pkl_sz) {
    return nullptr;
  ...
  if (details_json && strlen(details_json)) {
    ...
    boost::property_tree::read_json(ss, pt);
    PT_OPT_GET(nBits);
    PT_OPT_GET(minLength);
    PT_OPT_GET(maxLength);
  }
  auto fp = AtomPairs::getHashedAtomPairFingerprintAsBitVect(mol, nBits, minLength, maxLength);
  return std::unique_ptr<ExplicitBitVect>{fp};
}

#ifdef RDK_BUILD_AVALON_SUPPORT
std::unique_ptr<ExplicitBitVect> avalon_fp_helper(const char *mol_pkl,
                                                   size_t mol_pkl_sz,
                                                   const char *details_json) {
  if (!mol_pkl || !mol_pkl_sz) {
    return nullptr;
...
extern "C" char *get_topological_torsion_fp(const char *mol_pkl, size_t mol_pkl_sz,
                                const char *details_json) {
  auto fp = topological_torsion_fp_helper(mol_pkl, mol_pkl_sz, details_json);
  auto res = BitVectToText(*fp);
  return str_to_c(res);
}

extern "C" char *get_topological_torsion_fp_as_bytes(const char *mol_pkl, size_t mol_pkl_sz,
                                         size_t *nbytes,
                                         const char *details_json) {
  auto fp = topological_torsion_fp_helper(mol_pkl, mol_pkl_sz, details_json);
  auto res = BitVectToBinaryText(*fp);
...
extern "C" char *get_atom_pair_fp(const char *mol_pkl, size_t mol_pkl_sz,
                                const char *details_json) {
  auto fp = atom_pair_fp_helper(mol_pkl, mol_pkl_sz, details_json);
  auto res = BitVectToText(*fp);
  return str_to_c(res);
}

extern "C" char *get_atom_pair_fp_as_bytes(const char *mol_pkl, size_t mol_pkl_sz,
                                         size_t *nbytes,
                                         const char *details_json) {
  auto fp = atom_pair_fp_helper(mol_pkl, mol_pkl_sz, details_json);
  auto res = BitVectToBinaryText(*fp);
...
extern "C" char *get_avalon_fp(const char *mol_pkl, size_t mol_pkl_sz,
                                const char *details_json) {
  auto fp = avalon_fp_helper(mol_pkl, mol_pkl_sz, details_json);
  auto res = BitVectToText(*fp);
  return str_to_c(res);
}
