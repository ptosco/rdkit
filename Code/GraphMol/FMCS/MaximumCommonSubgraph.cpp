...
void MaximumCommonSubgraph::init() {
  ...
  for (size_t aj = 0; aj < nq; aj++) {
    ...
    if (MCSBondCompareAny ==
      ...
      unsigned int order = bond->getBondType();
...
bool checkIfRingsAreClosed(const Seed& fs, bool noLoneRingAtoms) {
  ...
  for (unsigned int ringIdx = 0; ringIdx < fragmentRings.size(); ++ringIdx) {
    ...
    for (auto bi : ri->bondRings().at(ringIdx)) {
      bool keepBond = false;
      for (auto memberOf : ri->bondMembers(bi)) {
        if (memberOf == ringIdx) {
          keepBond = true;
  ...
  if (noLoneRingAtoms) {
    for (const auto atom : fs.MoleculeFragment.Atoms) {
      auto ai = atom->getIdx();
      for (auto memberOf : ri->atomMembers(ai)) {
...
bool MaximumCommonSubgraph::growSeeds() {
  bool mcsFound = false;
  bool canceled = false;
  unsigned int steps = 99999;  // steps from last progress callback call. call
    ...
    si->grow(*this);
    {
      ...
      if (fs.CopyComplete) {
        ...
        if (possibleMCS && Parameters.BondCompareParameters.CompleteRingsOnly) {
          ...
          std::cerr << " fsAtomIdx ";
          std::set<unsigned int> atomIdxSet;
          std::set<unsigned int> bondIdxSet;
          for (const auto bond : fs.MoleculeFragment.Bonds) {
            atomIdxSet.insert(bond->getBeginAtomIdx());
            atomIdxSet.insert(bond->getEndAtomIdx());
            bondIdxSet.insert(bond->getIdx());
          }
          std::vector<unsigned int> atomIdxVector(atomIdxSet.begin(), atomIdxSet.end());
          std::sort(atomIdxVector.begin(), atomIdxVector.end());
          std::copy(atomIdxVector.begin(), atomIdxVector.end(), std::ostream_iterator<unsigned int>(std::cerr, ","));
          std::vector<unsigned int> bondIdxVector(bondIdxSet.begin(), bondIdxSet.end());
          std::sort(bondIdxVector.begin(), bondIdxVector.end());
          std::cerr << " fsBondIdx ";
          std::copy(bondIdxVector.begin(), bondIdxVector.end(), std::ostream_iterator<unsigned int>(std::cerr, ","));
          std::cerr << std::endl;
#endif
        }
        if (possibleMCS) {
...
bool MaximumCommonSubgraph::createSeedFromMCS(size_t newQueryTarget,
  ...
  for (match_V_t::const_iterator mit = match.begin(); mit != match.end();
       mit++) {
    unsigned int ai = mit->first;  // SeedAtomIdx in mcs seed
...
// use and update results of previous match stored in the seed
  ...
  */
  ...
  for (unsigned int newBondSeedIdx = match.MatchedBondSize;
    ...
    if (newBondOtherAtomSeedIdx <
      ...
      if (tb) {  // bond exists, check match with query molecule
        ...
        unsigned int qbi = seed.MoleculeFragment.BondsIdx[newBondSeedIdx];
  ...
  if (matched && Parameters.FinalMatchChecker) {
    ...
    c2.reserve(4096);
    for (unsigned int si = 0; si < seed.getNumAtoms();
